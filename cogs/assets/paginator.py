...
import discord
import random as r
from setup import SUPPORT_GUILD_INVITE
...
class Pages:
    ...
    def __init__(self, ctx, *, entries, per_page=12, show_entry_count=True):
        ...
        self.embed = discord.Embed(colour=r.randint(0, 0xFFFFFF)) # discord.Colour.blurple())
    ...
    async def numbered_page(self):
        ...
        try:
            ...
            to_delete.append(await self.channel.send("No page number detected"))
    ...
    async def show_help(self):
        ...
        self.embed.set_footer(text=f"Last page viewed: {self.current_page}.")
        await self.message.edit(embed=self.embed)
...
class HelpPaginator(Pages):
    ...
    async def from_cog(cls, ctx, cog):
        ...
        # get the commands
        entries = ctx.bot.get_cog_commands(cog_name)#sorted(ctx.bot.get_cog_commands(cog_name), key=lambda c: c.name)
    ...
    async def from_command(cls, ctx, command):
        try:
            entries = command.commands#sorted(command.commands, key=lambda c: c.name)
    ...
    async def from_bot(cls, ctx):
        ...
        entries = ctx.bot.commands#sorted(ctx.bot.commands, key=key)
        nested_pages = []
        per_page = 9
    ...
    async def show_help(self):
        ...
        self.embed.set_footer(
            text=f"Last page viewed: {self.current_page}."
        )
        await self.message.edit(embed=self.embed)
